...
import 'axis/axis_custom.dart' show AxisCustom, domainAxisKey, measureAxisKey;
...
abstract class BaseCartesianRenderer<D> extends BaseSeriesRenderer<D>
  ...
  void configureMeasureAxes(List<MutableSeries<D>> seriesList) {
    seriesList.forEach((MutableSeries<D> series) {
      ...
      final domainAxis = series.getAttr(domainAxisKey) as AxisCustom<D>?;
      final domainFn = series.domainFn;
      ...
      final measureAxis = series.getAttr(measureAxisKey) as AxisCustom<num>?;
      if (measureAxis == null) {
        return;
      }
  ...
  int findNearestViewportStart(
      AxisCustom<D> domainAxis, AccessorFn<D> domainFn, List<Object?> data) {
