...
import '../axis_custom.dart' show AxisOrientation;
...
abstract class BaseTickDrawStrategy<D> implements TickDrawStrategy<D> {
  ...
  void drawLabel(
    ...
    for (final line in labelElements) {
      ...
      // quannh edited 13/04/2022 sửa để thêm số :00 đằng sau giờ
      try {
        int cccc = int.parse(line.text);
        if (cccc > 0 && cccc < 24) {
          line.setText = '${line.text}';
        } else if (cccc >= 24 && cccc <= 30) {
          line.setText = '';
        }
      } catch (e) {
        print(e.toString());
