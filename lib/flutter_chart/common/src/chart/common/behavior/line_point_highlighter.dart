...
import '../../cartesian/axis/axis_custom.dart'
...
class _LinePointLayoutView<D> extends LayoutView {
  ...
  void paint(ChartCanvas canvas, double animationPercent) {
    ...
    for (final pointElement in points) {
      ...
      if (points.length >= 2) {
        int pos = points.indexOf(pointElement);
        if (pos == 1) {
          // quannh 05/05/2022 do chart line ở bên trên nên chỉ vẽ chart ở vị trí số 1
          drawValue(pointElement, canvas);
        }
      } else {
        drawValue(pointElement, canvas);
      }
  ...
  drawValue(var pointElement, var canvas){
